...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .route-table th {
    ...
    text-transform: uppercase; letter-spacing: 0.8px; color: ${theme.textMuted};
    padding: 6px 6px; text-align: left; border-bottom: 1px solid ${theme.border};
    overflow: hidden; white-space: nowrap;
  }
  .route-table td {
    padding: 6px 6px; border-bottom: 1px solid ${theme.border}22;
    color: ${theme.text}; vertical-align: middle;
  ...
  .icon-btn {
    ...
    font-size: 14px; padding: 3px 3px; border-radius: 6px;
  ...
  .live-dot {
    ...
    background: ${theme.success}; animation: pulse 1.5s ease-in-out infinite; flex-shrink: 0;
  }
  .live-address {
  ...
  .live-open-btn {
    ...
    font-family: 'DM Sans', sans-serif; padding: 10px 16px;
...
function LiveView({ address }) {
  ...
  return (
    <div className="live-view-card">
      ...
      <iframe className="live-view-map" src={embedUrl} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Live View χάρτης" />
      <button className="live-open-btn" onClick={() => window.open(mapsUrl, "_blank")}>🗺️ Άνοιγμα στο Google Maps</button>
    </div>
  );
}
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="btn-row">
          <button className="btn btn-success" style={{ marginBottom: 0 }}
            onClick={() => onSave({ ...route, end: { ...route.end, label: clientName.trim() || "Άγνωστο", location: address.trim() || route.end?.location } })}>
            ✓ ΑΠΟΘΗΚΕΥΣΗ
          </button>
