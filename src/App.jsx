...
// Αφαίρεση τόνων από κεφαλαία
const stripTones = (str) =>
  str
    .toUpperCase()
    .replace(/Ά/g, "Α").replace(/Έ/g, "Ε").replace(/Ή/g, "Η")
    .replace(/Ί/g, "Ι").replace(/Ϊ/g, "Ι").replace(/Ό/g, "Ο")
    .replace(/Ύ/g, "Υ").replace(/Ϋ/g, "Υ").replace(/Ώ/g, "Ω")
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .logo {
    ...
    background: linear-gradient(135deg, #e8edf5 0%, ${theme.accent} 100%);
  ...
  .btn-primary {
    background: linear-gradient(135deg, ${theme.primary} 0%, #1456c8 100%);
    color: white;
    box-shadow: 0 4px 20px rgba(29,110,245,0.35);
  }
  ...
  .route-table td { padding: 10px; border-bottom: 1px solid ${theme.border}22; color: ${theme.text}; vertical-align: top; }
  ...
  .del-btn { background: none; border: none; color: ${theme.danger}; cursor: pointer; font-size: 16px; padding: 4px; }
`;

// ─── Thor SVG ────────────────────────────────────────────────────
const ThorIcon = () => (
  <svg className="logo-icon" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect x="15.5" y="17" width="3" height="12" rx="1.2" fill="#38bdf8" opacity="0.85"/>
    <rect x="8" y="7" width="18" height="10" rx="2.5" fill="url(#hg)"/>
    <rect x="9" y="7.5" width="16" height="2" rx="1" fill="#38bdf8" opacity="0.35"/>
    <path d="M18.5 9.5 L16 13 L17.5 13 L15 17 L19.5 12.5 L17.5 12.5 Z" fill="white" opacity="0.9"/>
    <defs>
      <linearGradient id="hg" x1="8" y1="7" x2="26" y2="17" gradientUnits="userSpaceOnUse">
        <stop offset="0%" stopColor="#2d7fff"/>
        <stop offset="100%" stopColor="#0f3fa0"/>
      </linearGradient>
    </defs>
  </svg>
);
...
const reverseGeocode = async (lat, lon) => {
  try {
    ...
    return parts + (city ? `, ${city}` : "") || data.display_name || `${lat.toFixed(5)}, ${lon.toFixed(5)}`;
  } catch {
    return `${lat.toFixed(5)}, ${lon.toFixed(5)}`;
  }
};

// GPS key rounded to ~100m for matching
...
function ArrivalModal({ rawAddress, knownEntry, onDone, onCancel }) {
  ...
  // Already known location — just confirm
  if (step === "known") {
    return (
      <div className="overlay">
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ fontSize: 15, color: theme.accent, fontWeight: 600, marginBottom: 20 }}>
            👤 {knownEntry.name}
          </div>
  ...
  if (step === "edit_number") {
    // Keep street name, let user correct the number
        ...
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="btn-row">
            <button className="btn btn-primary" onClick={() => {
              const addr = editedNumber
                ? rawAddress.replace(/\s*\d+(\s*,)/, ` ${editedNumber}$1`).replace(/\s+\d+$/, ` ${editedNumber}`) || `${streetOnly} ${editedNumber}`
                : rawAddress;
        ...
        <div className="modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="input-group">
            ...
            <input
              className="input"
              type="text"
              placeholder="π.χ. Παπαδόπουλος Γιώργης"
              value={clientName}
          ...
          <div className="btn-row">
            <button className="btn btn-success" onClick={() => onDone(finalAddress, clientName.trim() || "Αγνωστο")}>
              ✓ ΑΠΟΘΗΚΕΥΣΗ
            </button>
            <button className="btn btn-secondary" onClick={() => setStep("confirm_address")}>ΠΙΣΩ</button>
          </div>
    ...
    <div className="overlay" onClick={onCancel}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="input-group">
          ...
          <input className="input" type="number" placeholder="45.5" value={form.liters} onChange={(e) => setForm({ ...form, liters: e.target.value })} />
        </div>
        <div className="input-group">
          <label className="input-label">Ποσό (€)</label>
          <input className="input" type="number" placeholder="82.00" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
        </div>
        <div className="input-group">
          <label className="input-label">Χιλιόμετρα (προαιρετικό)</label>
          <input className="input" type="number" placeholder="125450" value={form.km} onChange={(e) => setForm({ ...form, km: e.target.value })} />
        </div>
        <div className="btn-row">
...
export default function App() {
  ...
  // locations: { "lat,lon_key": { address: string, name: string } }
  ...
  useEffect(() => {
    ...
    setProfile(JSON.parse(localStorage.getItem("kt_profile") || "{}"));
  ...
  useEffect(() => {
    localStorage.setItem("kt_profile", JSON.stringify(profile));
  ...
  }, [profile, routes, allRoutes, locations, fuels]);
  ...
  const startFromBase = () => {
    setActiveRoute({ id: Date.now(), start: { location: profile.baseAddress || "Εδρα", time: now(), timestamp: Date.now() } });
  };

  const startFromGPS = async () => {
    const coords = await getCoords();
    let location = "Αγνωστη τοποθεσια";
    if (coords) location = await reverseGeocode(coords.lat, coords.lon);
    setActiveRoute({ id: Date.now(), start: { location, time: now(), timestamp: Date.now() } });
  };

  const endRoute = async () => {
    if (!activeRoute) return;
    const coords = await getCoords();
    let rawAddress = "Αγνωστη τοποθεσια";
    let key = null;
    if (coords) {
      rawAddress = await reverseGeocode(coords.lat, coords.lon);
      key = gpsKey(coords.lat, coords.lon);
    }
    const knownEntry = key && locations[key] ? locations[key] : null;
    setArrivalData({ rawAddress, key, knownEntry });
  };

  const handleArrivalDone = (finalAddress, clientName) => {
    if (!activeRoute) return;
    if (arrivalData.key) {
      setLocations((prev) => ({ ...prev, [arrivalData.key]: { address: finalAddress, name: clientName } }));
    }
  ...
  const saveFuel = (form) => {
    setFuels((f) => [...f, { id: Date.now(), ...form, date: todayKey }]);
    setShowFuel(false);
  };
  ...
  const exportExcel = () => {
    ...
    const totalKm = finalKm - Number(profile.startKm || 0);
    data.push({}, { "Εναρξη": "Συνολικος χρονος (λεπτα)", "Ωρα Εναρξης": Math.round(totalTime / 60000) }, { "Εναρξη": "Συνολικα χιλιομετρα", "Ωρα Εναρξης": totalKm });
  ...
  const driverName = [profile.firstName, profile.lastName].filter(Boolean).join(" ");
      ...
      <div className="app">
        ...
        <div className="header">
          <div className="header-top">
            ...
            {profile.plate && <div className="plate-badge">{profile.plate}</div>}
          </div>
          {driverName && <div className="driver-name">{driverName} · {todayKey}</div>}
        </div>
              ...
              {activeRoute ? (
                <div className="active-route-card">
                  <div style={{ display: "flex", alignItems: "center", marginBottom: 14 }}>
                    ...
                    <span style={{ fontFamily: "Syne,sans-serif", fontWeight: 700, fontSize: 15, color: theme.accent }}>
                      ΔΙΑΔΡΟΜΗ ΣΕ ΕΞΕΛΙΞΗ
                    </span>
                  </div>
              ...
              <div className="card">
                ...
                {routes.length === 0 ? (
                  ...
                  <table className="route-table">
                    <thead><tr><th>#</th><th>ΠΕΛΑΤΗΣ</th><th>ΩΡΑ</th></tr></thead>
                    <tbody>
                      {routes.map((r, i) => (
                        <tr key={i}>
            ...
            <div>
              ...
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                {routes.length === 0 ? (
                  ...
                  <table className="route-table">
                    <thead>
                      <tr><th>#</th><th>ΠΕΛΑΤΗΣ</th><th>ΕΝΑΡΞΗ</th><th>ΑΦΙΞΗ</th></tr>
                    </thead>
                    <tbody>
                      {routes.map((r, i) => (
                        <tr key={i}>
            ...
            <div>
              ...
              <div className="stat-grid">
                <div className="stat-box">
                  ...
                  <div className="stat-value">{applyFilters(allRoutes).length}<span className="stat-unit">δρομ.</span></div>
                </div>
                <div className="stat-box">
                  <div className="stat-label">ΣΗΜΕΡΑ</div>
                  <div className="stat-value">{routes.length}<span className="stat-unit">δρομ.</span></div>
                </div>
              </div>
              ...
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                {filteredRoutes.length === 0 ? (
                  ...
                  <table className="route-table">
                    <thead><tr><th>#</th><th>ΠΕΛΑΤΗΣ</th><th>ΗΜ/ΝΙΑ</th><th>ΩΡΑ</th></tr></thead>
                    <tbody>
            ...
            <div>
              ...
              <div className="stat-grid">
                <div className="stat-box">
                  ...
                  <div className="stat-value">{totalFuelCost.toFixed(1)}<span className="stat-unit">€</span></div>
                </div>
                <div className="stat-box">
                  <div className="stat-label">ΣΥΝΟΛΙΚΑ ΛΙΤΡΑ</div>
                  <div className="stat-value">{totalFuelLiters.toFixed(1)}<span className="stat-unit">L</span></div>
                </div>
              </div>
              <div className="card" style={{ padding: 0, overflow: "hidden" }}>
                {fuels.length === 0 ? (
                  ...
                  <table className="route-table">
                    <thead><tr><th>ΗΜ/ΝΙΑ</th><th>ΛΙΤΡΑ</th><th>ΠΟΣΟ</th><th>ΧΛΜ</th></tr></thead>
                    <tbody>
                      {fuels.map((f) => (
                        <tr key={f.id}>
              ...
              <div className="card">
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div className="input-group">
                    ...
                    <input className="input" placeholder="Γιώργης" value={profile.firstName || ""} onChange={(e) => setProfile({ ...profile, firstName: e.target.value })} />
                  </div>
                  <div className="input-group">
                    <label className="input-label">Επίθετο</label>
                    <input className="input" placeholder="Παπαδόπουλος" value={profile.lastName || ""} onChange={(e) => setProfile({ ...profile, lastName: e.target.value })} />
                  </div>
                </div>
              </div>
              <div className="card">
                ...
                <div className="input-group">
                  ...
                  <input className="input" placeholder="ΑΒΓ-1234" value={profile.plate || ""} onChange={(e) => setProfile({ ...profile, plate: e.target.value })} />
                </div>
                <div className="input-group">
                  <label className="input-label">Χιλιόμετρα έναρξης ημέρας</label>
                  <input className="input" type="number" placeholder="125000" value={profile.startKm || ""} onChange={(e) => setProfile({ ...profile, startKm: e.target.value })} />
                </div>
              </div>
              <div className="card">
                ...
                <div className="input-group">
                  ...
                  <input className="input" placeholder="Αθήνα, Ελλάδα" value={profile.baseAddress || ""} onChange={(e) => setProfile({ ...profile, baseAddress: e.target.value })} />
                </div>
              </div>
